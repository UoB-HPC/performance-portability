...
local-1036)
    ...
    rm -rf BabelStream
    (
        bench_exec exec_build gfx1036-local $LLVM omp std-indices
        bench_exec exec_build gfx1036-local $ROCM hip kokkos
        bench_exec exec_build gfx1036-local $ROC_STDPAR_INTERPOSE std-indices
        wait
    )
    for xnack in 1 0; do
        export HSA_XNACK="$xnack"
        bench_exec exec_submit gfx1036-local $LLVM omp std-indices
        bench_exec exec_submit gfx1036-local $ROCM hip kokkos
        bench_exec exec_submit gfx1036-local $ROC_STDPAR_INTERPOSE std-indices
    done
...
local-vii)
    ...
    rm -rf BabelStream
    (
        bench_exec exec_build radeonvii-local $LLVM omp std-indices
    ...
    for xnack in 1 0; do
        export HSA_XNACK="$xnack"
        bench_exec exec_submit radeonvii-local $LLVM omp std-indices
        bench_exec exec_submit radeonvii-local $ROCM hip kokkos
        bench_exec exec_submit radeonvii-local $ROC_STDPAR_INTERPOSE std-indices
    done
