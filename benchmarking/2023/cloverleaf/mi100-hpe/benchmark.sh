...
omp)
  ...
  append_opts "-DOFFLOAD=ON -DOFFLOAD_FLAGS=-fopenmp;--offload-arch=gfx908;-Ofast;-fopenmp-target-fast"
...
std-indices)
  ...
  roc-stdpar-interpose-*)
    ...
    append_opts "-DCXX_EXTRA_FLAGS=--hipstdpar;--hipstdpar-path=$HOME/roc-stdpar/include;--hipstdpar-prim-path=/opt/rocm-5.3.3/rocprim/include;--hipstdpar-thrust-path=/opt/rocm-5.3.3/rocthrust/include;--hipstdpar-interpose-alloc;--offload-arch=gfx908;-march=native;-Ofast"
    ;;
  roc-stdpar-*)
    append_opts "-DCLANG_STDPAR_PATH=$HOME/roc-stdpar/include"
    append_opts "-DCXX_EXTRA_FLAGS=--hipstdpar;--hipstdpar-path=$HOME/roc-stdpar/include;--hipstdpar-prim-path=/opt/rocm-5.3.3/rocprim/include;--hipstdpar-thrust-path=/opt/rocm-5.3.3/rocthrust/include;--offload-arch=gfx908;-march=native;-Ofast"
    ;;
  esac
  ;;
sycl-acc)
