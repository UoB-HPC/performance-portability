...
handle_cmd "${1}" "${2}" "${3}" "cloverleaf" "h100_80g" "${INPUT_BM:-}_${STAGE:-}"
...
kokkos)
  ...
  append_opts "-DKokkos_ARCH_HOPPER90=ON"
...
cuda)
  ...
  append_opts "-DCUDA_ARCH=sm_90"
  append_opts "-DCXX_EXTRA_FLAGS=-march=znver3"
  BENCHMARK_EXE="cuda-cloverleaf"
  ;;
omp)
  append_opts "-DMODEL=omp-target"
  append_opts "-DOFFLOAD=ON -DOFFLOAD_FLAGS=-mp=gpu;-gpu=cc90"
  append_opts "-DRELEASE_FLAGS='' -DCXX_EXTRA_FLAGS=-mp=gpu;-gpu=cc90;-O3;-tp=zen3"
