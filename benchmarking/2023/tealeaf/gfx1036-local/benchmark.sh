...
handle_cmd "${1}" "${2}" "${3}" "tealeaf" "gfx1036" "bm_${INPUT_BM:-}_xnack_${HSA_XNACK:-}_utpx_${UTPX:-}"
...
hip)
  ...
  append_opts "-DCXX_EXTRA_FLAGS=--offload-arch=gfx1036"
...
omp)
  ...
  append_opts "-DOFFLOAD=ON -DOFFLOAD_FLAGS=-fopenmp;--offload-arch=gfx1036;$extra"
...
std-indices)
  ...
  hipsycl-*)
    ...
    ;;
  oneapi-*)
    hip_sycl_flags="-fsycl-targets=amdgcn-amd-amdhsa;-Xsycl-target-backend;--offload-arch=gfx1036"
  ...
  roc-stdpar-interpose-*)
    ...
    append_opts "-DCXX_EXTRA_FLAGS=--hipstdpar;--hipstdpar-path=$HOME/roc-stdpar/include;--hipstdpar-prim-path=/opt/rocm-5.3.3/rocprim/include;--hipstdpar-thrust-path=/opt/rocm-5.3.3/rocthrust/include;--hipstdpar-interpose-alloc;--offload-arch=gfx1036;-march=native"
    ;;
  roc-stdpar-*)
    append_opts "-DCLANG_STDPAR_PATH=$HOME/roc-stdpar/include"
    append_opts "-DCXX_EXTRA_FLAGS=--hipstdpar;--hipstdpar-path=$HOME/roc-stdpar/include;--hipstdpar-prim-path=/opt/rocm-5.3.3/rocprim/include;--hipstdpar-thrust-path=/opt/rocm-5.3.3/rocthrust/include;--offload-arch=gfx1036;-march=native"
    ;;
  esac
  ;;
sycl-acc)
