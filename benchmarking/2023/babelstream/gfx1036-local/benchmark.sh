...
handle_cmd "${1}" "${2}" "${3}" "babelstream" "gfx1036" "xnack_${HSA_XNACK:-}_utpx_${UTPX:-}"
...
std-indices)
  ...
  case "$COMPILER" in
  llvm-*)
    append_opts "-DCXX_EXTRA_FLAGS=-fopenmp;-stdlib=libc++;-fexperimental-library;--offload-arch=gfx1036"
  ...
  oneapi-*)
    ...
    append_opts "-DCXX_EXTRA_FLAGS=-fsycl;-fsycl-targets=amdgcn-amd-amdhsa;-Xsycl-target-backend;--offload-arch=gfx1036;-march=native"
    append_opts "-DCXX_EXTRA_LIBRARIES=tbb"
    ;;
  roc-stdpar-interpose-*)
    append_opts "-DCXX_EXTRA_FLAGS=--hipstdpar;--hipstdpar-path=$HOME/roc-stdpar/include;--hipstdpar-interpose-alloc;--offload-arch=gfx1036;-march=native"
    ;;
  roc-stdpar-*)
    append_opts "-DCXX_EXTRA_FLAGS=--hipstdpar;--hipstdpar-path=$HOME/roc-stdpar/include;--offload-arch=gfx1036;-march=native"
    ;;
  *) unknown_compiler ;;
  esac
