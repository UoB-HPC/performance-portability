...
cuda)
  ...
  append_opts 'COMPILER=GNU NV_ARCH=VOLTA CODE_GEN_VOLTA="-gencode arch=compute_70,code=sm_70"'
...
omp)
  ...
  append_opts "-DOMP_OFFLOAD_FLAGS=-target=gpu;-gpu=cc70;--restrict;-fast;-Mllvm-fast;-Ktrap=none;-Minfo=accel;-Minfo=mp"
  ;;
std-indices)
  export USE_MAKE=false
  fetch_src "stdpar"
  ...
  append_opts "-DCMAKE_C_COMPILER=$NVHPC_PATH/compilers/bin/nvc"
  append_opts "-DCMAKE_CXX_COMPILER=$NVHPC_PATH/compilers/bin/nvc++"
  append_opts "-DNVHPC_OFFLOAD=cc70 -DCXX_EXTRA_FLAGS=-fast -DSERIAL_COPY_CTOR=ON"
  ;;
*) unknown_model ;;
esac
